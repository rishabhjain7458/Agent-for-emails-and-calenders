...
function bodyFromPayload(payload) {
    ...
    const text = parts.find((part) => part.mimeType === 'text/plain') ?? parts.find((part) => part.mimeType === 'text/html');
...
export async function listEmails(userId, query = 'in:inbox', maxResults = 20, pageToken) {
...
export async function sendReply(userId, input) {
    const auth = await getAuthorizedGoogleClient(userId);
    const gmail = google.gmail({ version: 'v1', auth });
