...
export async function exchangeMicrosoftCode(code) {
    ...
    if (!profile.id || !email)
        throw new HttpError(400, 'Microsoft profile is incomplete');
    return {
        profile: {
            id: profile.id,
            email,
...
export async function getMicrosoftAccessToken(userId) {
    ...
    if (accessToken && expiresAt > Date.now() + 60_000)
        return accessToken;
    if (!refreshToken)
        throw new HttpError(401, 'Microsoft refresh token is missing. Please sign in again.');
...
export function redirectWithMicrosoftSession(token) {
