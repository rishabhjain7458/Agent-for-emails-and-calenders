...
export async function upsertExternalTasks(tenantId: string, userId: string, tasks: ExternalTask[]) {
  for (const task of tasks) {
    ...
    if (existing.rowCount) {
      await pool.query(
        'UPDATE tasks SET title = $1, due_date = $2, status = $3, google_task_list_id = COALESCE($4, google_task_list_id), updated_at = NOW() WHERE id = $5',
        [task.title, task.dueDate ?? null, task.status ?? 'pending', task.googleTaskListId ?? null, existing.rows[0].id]
      );
      continue;
    }

    await pool.query(
      'INSERT INTO tasks (tenant_id, user_id, title, due_date, status, google_task_id, google_task_list_id) VALUES ($1, $2, $3, $4, $5, $6, $7)',
      [tenantId, userId, task.title, task.dueDate ?? null, task.status ?? 'pending', task.googleTaskId, task.googleTaskListId ?? null]
    );
  }
}
