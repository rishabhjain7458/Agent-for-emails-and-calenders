...
export async function updateSettings(tenantId: string, userId: string, input: {
  geminiApiKey?: string;
  timezone?: string;
  ...
  const result = await pool.query(
    ...
    VALUES ($1, $2, $3, COALESCE($4, 'Asia/Kolkata'), COALESCE($5::jsonb, '{"priorityCategories":["security","financial","work","meetings"],"ignorePromotions":true}'::jsonb))
