...
import { Alert, Box, Button, Card, CardContent, Chip, Dialog, DialogActions, DialogContent, DialogTitle, Grid, LinearProgress, MenuItem, Stack, TextField, ToggleButton, ToggleButtonGroup, Typography } from '@mui/material';
...
export function CalendarPage() {
  ...
  return (
    <>
      <PageHeader title="Calendar" subtitle="Combined events, quick scheduling, and a clearer agenda." action={<Button variant="outlined" startIcon={<RefreshIcon />} onClick={load}>Refresh</Button>} />
        ...
        <Grid item xs={12}>
          <Card className="premium-panel">
            <CardContent sx={{ p: { xs: 1.5, md: 2 } }}>
              <Stack direction={{ xs: 'column', md: 'row' }} alignItems={{ xs: 'flex-start', md: 'center' }} justifyContent="space-between" spacing={1.25} sx={{ mb: 1.5 }}>
                <Box>
                  <Typography variant="h6" sx={{ fontSize: { xs: '1rem', md: '1.08rem' } }}>Quick Create</Typography>
                  <Typography color="text.secondary" variant="body2">Tap a slot to prefill time, then finish the details.</Typography>
                </Box>
                <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap>
                  <Chip size="small" label={isCombined ? 'Primary calendar' : activeSpace?.email ?? 'Selected calendar'} color="primary" variant="outlined" />
                  <Chip size="small" icon={<AccessTimeIcon />} label={form.timezone.replace('_', ' ')} variant="outlined" />
                </Stack>
              </Stack>
              <Grid container spacing={1.25} alignItems="flex-start">
                <Grid item xs={12} md={3}>
                  <TextField size="small" fullWidth label="Meeting title" value={form.title} onChange={(event) => updateForm('title', event.target.value)} placeholder="Project discussion" />
                </Grid>
                <Grid item xs={12} sm={4} md={2}>
                  <TextField size="small" fullWidth label="Date" type="date" value={form.date} InputLabelProps={{ shrink: true }} onChange={(event) => updateForm('date', event.target.value)} />
                </Grid>
                <Grid item xs={6} sm={4} md={1.5}>
                  <TextField size="small" fullWidth label="Start" type="time" value={form.startTime} InputLabelProps={{ shrink: true }} onChange={(event) => updateForm('startTime', event.target.value)} />
                </Grid>
                <Grid item xs={6} sm={4} md={1.5}>
                  <TextField size="small" fullWidth label="End" type="time" value={form.endTime} InputLabelProps={{ shrink: true }} onChange={(event) => updateForm('endTime', event.target.value)} />
                </Grid>
                <Grid item xs={12} md={3}>
                  <TextField size="small" fullWidth select label="Country / timezone" value={form.timezone} onChange={(event) => updateForm('timezone', event.target.value)}>
                ...
                <Grid item xs={12} md={3.5}>
                  <TextField size="small" fullWidth label="Attendees" value={form.attendees} onChange={(event) => updateForm('attendees', event.target.value)} placeholder="name@example.com, teammate@example.com" />
                </Grid>
                <Grid item xs={12} md={4.5}>
                  <TextField size="small" fullWidth label="Description" value={form.description} onChange={(event) => updateForm('description', event.target.value)} placeholder="Agenda, Meet link, or context" />
                </Grid>
                <Grid item xs={12} md={2}>
                  <Button fullWidth disabled={creating} variant="contained" startIcon={<AddIcon />} onClick={() => submit(false)} sx={{ minHeight: 40 }}>
          ...
          <Card className="premium-panel">
            <CardContent sx={{ p: { xs: 1.5, md: 2 } }}>
              <Stack direction={{ xs: 'column', sm: 'row' }} alignItems={{ xs: 'stretch', sm: 'center' }} justifyContent="space-between" spacing={1.25} sx={{ mb: 2 }}>
                <Box>
                  <Typography variant="h6">Combined Schedule</Typography>
              ...
              <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap sx={{ mt: 1.25 }}>
                ...
                <Grid item xs={12} lg={3}>
                  <Box sx={{ border: '1px solid', borderColor: 'divider', borderRadius: 2.5, bgcolor: 'background.default', p: 1.5, position: { lg: 'sticky' }, top: 96 }}>
                    <Typography variant="h6" sx={{ mb: 1.5, fontSize: '1rem' }}>Agenda</Typography>
