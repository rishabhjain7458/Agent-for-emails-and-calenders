...
export async function getSocialConnectUrl(platform: 'facebook' | 'instagram' | 'linkedin' | 'x' | 'reddit', mobile = false, origin?: string) {
  const { data } = await api.get<{ data: { url: string; redirectUri: string } }>(`/auth/social/${platform}/connect`, {
    params: { ...(mobile ? { mobile: '1' } : {}), ...(origin ? { origin } : {}) }
  });
  return data.data;
}
